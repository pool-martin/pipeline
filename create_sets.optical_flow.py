...
''' 
Create Splits for optical flow to be used in 2D or 3D CNN models. 
...
def select_frames_for_optical_flow(frames_snippet, init_bound, end_bound, args):
    optical_flow_frames = []
    for frames in frames_snippet:
        frame_1 = frames+1 if frames+1 < end_bound else end_bound
        frame_5 = frames+5 if frames+5 < end_bound else end_bound
        frame_15 = frames+15 if frames+15 < end_bound else end_bound
        optical_flow_frames.append('{}|{}|{}|{}|{}'.format(frames, frame_1, frame_5, frame_15, end_bound))
...
def generate_snippet(video_name,frame_entry, split_type, frame_count, fps, etf_file, labels, frame_position, position, args, is_test_split):
    ...
    optical_flow_frames = select_frames_for_optical_flow(frames_snippet, init_bound, end_bound, args)
    #print('$', end='', flush=True)
    ...
    # print('snippet_folder:', snippet_folder, flush=True)
    ...
    with open(snippet_path, "w") as f:
        for item in optical_flow_frames:
                f.write("%s\n" % item)
    return True
...
def create_splits(args):
    ...
    full_dir_path = os.path.join(args.output_path, args.split_number, 'of', '{}_fps'.format(args.sample_rate), args.engine_type, 'w_{}_l_{}'.format(args.snippet_width, args.snippet_length))
    command = "mkdir -p " + full_dir_path
    print('\n', command)
    call(command, shell=True)

    create_video_split('network_training_set', 'of', positive_network_training_set, negative_network_training_set, args)
    create_video_split('network_validation_set', 'of', positive_network_validation_set, negative_network_validation_set, args)
    create_video_split('svm_training_set', 'of', positive_svm_training_set, negative_svm_training_set, args)
    create_video_split('svm_validation_set', 'of', positive_svm_validation_set, negative_svm_validation_set, args)

    print('################################ TEST SET ####################################')

    create_video_split('test_set', 'of', positive_test_set, negative_test_set, args, split_test=True)
