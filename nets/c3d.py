...
def C3D(input, num_classes, keep_pro=0.2):
    ...
    with tf.variable_scope('C3D'):
        ...
        #                     stride=[1, 1, 1]
        #                     ):
            net = tf.layers.conv3d(input, 64, kernel_size=[3, 3, 3], strides=[1, 1, 1], padding='SAME', activation=tf.nn.relu)
            # net = slim.conv3d(input, 64, scope='conv1')
            end_points['conv1'] = net
            net = tf.layers.max_pooling3d(net, pool_size=[1, 2, 2], strides=[ 1, 2, 2], padding='VALID')
            # net = slim.max_pool3d(net, kernel_size=[1, 2, 2], stride=[1, 2, 2], padding='VALID', scope='max_pool1')
            end_points['max_pool1'] = net
            net = tf.layers.conv3d(net, 128, kernel_size=[3, 3, 3], strides=[1, 1, 1], padding='SAME', activation=tf.nn.relu)
            # net = slim.conv3d(net, 128, scope='conv2')
            end_points['conv2'] = net
            net = tf.layers.max_pooling3d(net, pool_size=[2, 2, 2], strides=[ 2, 2, 2], padding='VALID')
            # net = slim.max_pool3d(net, kernel_size=[2, 2, 2], stride=[2, 2, 2], padding='VALID', scope='max_pool2')
            end_points['max_pool2'] = net
            net = tf.layers.conv3d(net, 256, kernel_size=[3, 3, 3], strides=[1, 1, 1], padding='SAME', activation=tf.nn.relu)
            net = tf.layers.conv3d(net, 256, kernel_size=[3, 3, 3], strides=[1, 1, 1], padding='SAME', activation=tf.nn.relu)
            ...
            net = tf.layers.conv3d(net, 512, kernel_size=[3, 3, 3], strides=[1, 1, 1], padding='SAME', activation=tf.nn.relu)
            # net = slim.conv3d(net, 512, scope='conv4a')
            end_points['conv4a'] = net
            ...
            end_points['padding'] = net
            net = tf.layers.max_pooling3d(net, pool_size=[1, 2, 2], strides=[ 1, 3, 3], padding='VALID')
            # net = slim.max_pool3d(net, kernel_size=[1, 2, 2], stride=[1, 3, 3], padding='VALID', scope='max_pool5')
            ...
            net = tf.layers.dense(net, 4096, activation=tf.nn.relu)
            end_points['fc7'] = net
            net = tf.layers.dropout(net, rate=keep_pro)
