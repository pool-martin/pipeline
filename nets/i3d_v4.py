  ...
  def _build(self, inputs, is_training):
    ...
    if self._tf_library == 'sonnet':
        ...
        net = slim.conv3d(inputs, 
                        ...
                        padding=self._padding,
                        ### use_bias=False,
                        normalizer_fn = tf.contrib.layers.batch_norm(inputs, is_training=is_training, reuse = self._reuse) if self._use_batch_norm else None,
        ...
        with tf.variable_scope(scope, 'BlockReductionB'):
            with tf.variable_scope('Branch_0'):
                branch_0 = Unit3D(output_channels=192, kernel_shape=[1, 1, 1],
                        name='Conv3d_0a_1x1')(inputs, is_training=is_training)
                branch_0 = Unit3D(output_channels=192, kernel_shape=[3, 3, 3], stride=[1, 2, 2, 2, 1], 
                                  padding='SAME', name='Conv3d_0b_3x3')(branch_0, is_training=is_training)
        ...
        with tf.variable_scope(scope, 'BlockInceptionC'):
            ...
            with tf.variable_scope('Branch_2'):
                ...
                branch_2 = tf.concat(axis=4, values=[
                            ...
                            Unit3D(output_channels=256, kernel_shape=[1, 1, 3], name='Conv3d_0e_3x1')(branch_2, is_training=is_training)
                            ])
                            # Unit3D(output_channels=256, kernel_shape=[1, 1, 3],
    ...
    def _build(self, inputs, is_training, dropout_keep_prob=1.0):
        ...
        with tf.variable_scope('InceptionV4'):
            ...
            net = Unit3D(output_channels=32, kernel_shape=[3, 3, 3], stride=[2, 2, 2],
                        padding='SAME', name=end_point)(inputs, is_training=is_training)
            ...
            net = Unit3D(output_channels=32, kernel_shape=[3, 3, 3],
                        padding='SAME', name=end_point)(net, is_training=is_training)
            if add_and_check_final(end_point, net): return net, end_points
            ...
            with tf.variable_scope('Mixed_3a'):
                ...
                with tf.variable_scope('Branch_1'):
                    branch_1 =  Unit3D(output_channels=96, kernel_shape=[3, 3, 3], stride=[1, 2, 2],
                                    padding='SAME', name='Conv2d_0a_3x3')(net, is_training=is_training)
            ...
            with tf.variable_scope('Mixed_5a'):
                with tf.variable_scope('Branch_0'):
                    branch_0 =  Unit3D(output_channels=192, kernel_shape=[3, 3, 3], stride=[1, 2, 2], padding='SAME', name='Conv2d_1a_3x3')(net, is_training=is_training)
                with tf.variable_scope('Branch_1'):
